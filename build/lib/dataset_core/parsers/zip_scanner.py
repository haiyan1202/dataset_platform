...
class ArchiveReader:
    """Format-neutral, read-only archive access without extracting host paths."""
...
class _SevenZipArchive(ArchiveReader):
    ...
    def read(self, entry: ArchiveEntry) -> bytes:
        # py7zr reads through a WriterFactory. Request one member at a time to
        # avoid extracting archive paths onto the Worker host filesystem.
        self._archive.reset()
        factory = BytesIOFactory(max(entry.size, 1))
        self._archive.extract(targets=[entry.name], factory=factory)
        try:
